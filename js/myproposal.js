let MyProposal = {
  ...
  genPageData() {
    ...
    //先获取到所有到投票id
    let pk = $('.address').val();
    let mainPkr =  that.mainPKr[pk];
    let allProposalIdArray =  getMyProposalId(mainPkr);
    allProposalIdArray = allProposalIdArray.sort(sortNumber);
    if (  allProposalIdArray.length == 0 ) {
